import requests
APIKEY = "bRCR4gQR2nycslIQj9x3H3easaCFitDN4SDASEiliUfJJPZKJc-w0ZGBmaDo02WY"
...
def test_create_positive():
    ...
    respons = requests.post(url=URL+"/api-v2/projects", headers=myheaders, json=body)
...
def test_updatecreate_positive():
    ...
    respons = requests.put(url=f"{URL}/api-v2/projects/{id}", headers=myheaders, json=body)
    assert respons.status_code == 200
    #Получаем проект по ID
    respons = requests.get(url=f"{URL}/api-v2/projects/{id}", headers=myheaders)
    respons_body = respons.json()
